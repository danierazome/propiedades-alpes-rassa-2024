...
from src.modulos.planos.aplicacion.saga.saga_auditar_planos import CoordinadorCompensacionAuditoriaPlanos


@dataclass
class SagaCompensacionEvento(Evento):
    evento_dominio: EventoDominio


class SagaCompensacionHandler(EventHandler):

    def handle(self, event: SagaCompensacionEvento):
        from src.config.db import db
        print('SAGA COMPENSACION HANDLERR')
        coordinador = CoordinadorCompensacionAuditoriaPlanos()
        coordinador.inicializar_pasos(db=db)
        coordinador.procesar_evento(evento=event.evento_dominio)


@evento.register(SagaCompensacionEvento)
def ejecutar_conando_actualizar_plano(evento: SagaCompensacionEvento, app):
    with app.app_context():
        handler = SagaCompensacionHandler()
